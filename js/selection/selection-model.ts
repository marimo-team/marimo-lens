...
function chartPartMatchScore(unit: SelectionModelUnit, hit: SemanticHit): number | null {
  ...
  score += fieldScores;
  if (normalize(unitPart.label) === normalize(hitPart.label)) score += 12;
  if (
    unitPart.detail &&
    hitPart.detail &&
    normalize(unitPart.detail) === normalize(hitPart.detail)
  ) {
    score += 10;
  }
  return score;
...
function matchingFieldScore(
  ...
  return normalize(left[key]) === normalize(right[key]) ? value : null;
}
