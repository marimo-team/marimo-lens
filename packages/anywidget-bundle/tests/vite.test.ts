...
describe("anywidget bundle Vite plugin", () => {
  test("builds a configured artifact layout and manifest-backed module graph", async () => {
    ...
    expect(
      manifest.modules.every((path) => path.startsWith("modules/") && path.endsWith(".mjs")),
    ).toBe(true);
    await init;
    ...
    expect((await stat(join(fixture.outDir, manifest.style ?? "missing-style"))).isFile()).toBe(
      true,
    );
  });
  ...
  test("empties an output directory outside the Vite root", async () => {
    ...
    const outDir = await mkdtemp(join(tmpdir(), "anywidget-bundle-vite-output-"));
  ...
  test("serves an app file outside the Vite root through /@fs/", async () => {
    const fixture = await createFixture({});
    const appRoot = await mkdtemp(join(tmpdir(), "anywidget-bundle-vite-app-"));
...
async function createFixture(files: Readonly<Record<string, string>>) {
  const root = await mkdtemp(join(tmpdir(), "anywidget-bundle-vite-"));
