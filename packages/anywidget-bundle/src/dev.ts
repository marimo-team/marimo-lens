...
async function restartModel<ModelState extends AnyWidgetState>(
  ...
  if (state.disposed) return;
  await settleOperations(
    views.filter((view) => !view.disposed).map((view) => scheduleViewRender(state, view)),
  );
}
...
function cleanupView<ModelState extends AnyWidgetState>(
  view: ViewState<ModelState>,
): Promise<void> {
...
function cleanupModel<ModelState extends AnyWidgetState>(
  state: DevelopmentModel<ModelState>,
): Promise<void> {
...
async function settleViewGeneration<ModelState extends AnyWidgetState>(
  view: ViewState<ModelState>,
): Promise<void> {
...
async function settleListenerCleanup(
  model: ListenerModel | undefined,
