...
export function notebookSources(
  ...
  const candidates = element.matches(SOURCE_SELECTOR)
    ? [element]
    : [element, ...element.querySelectorAll(SOURCE_SELECTOR)];
  for (const candidate of candidates) {
    ...
    if (references !== null) {
      const hosts = referencedSources(candidate, documentIds);
...
export function referencedSources(
  ...
  const byId = documentIds ?? indexDocumentIds(element.ownerDocument);
