...
import pytest
from marimo_lens._anywidget_bundle import (
    Bundle,
    BundleArtifactError,
    BundledWidget,
)
...
def test_bundle_uses_configured_dev_server_entry(
    ...
    bundle = Bundle(
        static_dir=tmp_path,
        dev_server_env="MARIMO_LENS_TEST_DEV_SERVER",
        dev_entry=dev_entry,
    )
    monkeypatch.setenv("MARIMO_LENS_TEST_DEV_SERVER", "127.0.0.1:5173/")
...
def test_bundle_preserves_dev_server_base_path(
    ...
    monkeypatch.setenv("MARIMO_LENS_TEST_DEV_SERVER", "https://example.test/docs/")
